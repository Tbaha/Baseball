...
public class BaseballStat {
	...
	public double  BA() {
		double ba = (double) (this.Hits) / this.AB;
		return ba;
		
	}
	public double OBP() {
		int singles = this.Hits - this.dbl - this.triple - this.HR;	
		double obp = (double) (singles + this.dbl + this.triple + this.HR + this.BB + this.HBP) / (this.AB + this.BB + this.HBP + this.SF);
		return obp;
	}
	public double OBS() {
	double obs = (double) this.OBP() + this.SLG();
	return obs;
		
	}
	public double TB() {
		int singles = this.Hits - this.dbl - this.triple - this.HR;	
		double tb = (double) (singles + (2 * this.dbl) + (3 * this.triple) + (4 * this.HR));
		return tb;
	}
}
